...
const ContactInfoItem = ({ icon, text }) => (
  <div className="flex items-start space-x-3 text-sm text-gray-600">
    <span className="text-xl text-blue-600 flex-shrink-0">{icon}</span>
    <p>{text}</p>
  </div>
);
...
const Illustration = () => (
  <div className="flex flex-col items-center space-y-4 pt-8 md:pt-0">
    ...
    <div className="w-40 h-40 bg-blue-100 rounded-full flex items-center justify-center">
      <span className="text-5xl">📦🚴‍♂️</span> {/* Emoji placeholder */}
    </div>
    ...
    <div className="space-y-4 w-full">
      ...
      {/* 2. Delivery Area Requirements */}
      <div className="pt-4 space-y-2">
        <h4 className="text-sm font-bold text-gray-700">DELIVERY REQUIREMENTS</h4>
        <ContactInfoItem icon="🗺️" text="Must specify the primary delivery area (Ward) of operation in the Address field." />
        <ContactInfoItem icon="🚲" text="Own vehicle (bicycle/motorcycle) is required for delivery." />
        <ContactInfoItem icon="⏰" text="Must be available for flexible delivery hours including weekends." />
      </div>
    </div>
  </div>
);

const DeliveryBoyApplicationForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:4321/api/user/apply-delivery-boy",
        formData
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-10 flex flex-col items-center">
      ...
      {/* Top Header / Branding Section (BLUE) */}
      <div className="absolute top-0 w-full h-40 bg-gradient-to-r from-blue-600 to-indigo-600 shadow-lg">
          ...
          <div className="md:border-r md:pr-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <input
                ...
                onChange={handleChange}
                placeholder="Enter your address / primary delivery area (e.g., Ward 5)"
                className="w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition duration-150"
                required
              />
              ...
              <button
                type="submit"
                className="w-full bg-blue-600 text-white font-semibold py-3 mt-4 rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 transition duration-300 transform hover:scale-[1.005]"
              >
                Submit Application
              </button>
            </form>
...
export default DeliveryBoyApplicationForm;